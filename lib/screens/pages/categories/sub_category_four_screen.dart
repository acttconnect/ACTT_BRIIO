...
class SubCategoryFourScreen extends StatefulWidget {
  ...
  final String subCategoryThreeName;
  final List<dynamic>? siblingCategories; // Added for tabs

  const SubCategoryFourScreen({
    super.key,
    required this.categoryId,
    required this.subCategoryId,
    this.subSubCategoryId,
    required this.subCategoryThreeId,
    required this.subCategoryThreeName,
    this.siblingCategories,
  });

  @override
  State<SubCategoryFourScreen> createState() => _SubCategoryFourScreenState();
}

class _SubCategoryFourScreenState extends State<SubCategoryFourScreen> {
  List<dynamic> subCategoryFourList = [];
  List<Product> productList = [];
  List<Product> allProductsList = [];
  List<dynamic> allSubCategoryThree = [];
  ...
  void _scrollToSelected() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (_categoryKeys.containsKey(currentSubCategoryThreeId) && _categoryKeys[currentSubCategoryThreeId]?.currentContext != null) {
        Scrollable.ensureVisible(
          _categoryKeys[currentSubCategoryThreeId]!.currentContext!,
  ...
  void initState() {
    ...
    if (widget.siblingCategories != null) {
      ...
      for (var sibling in allSubCategoryThree) {
        final id = sibling['id'] ?? 0;
        _categoryKeys[id] = GlobalKey();
      }
    }

    _fetchSubCategoryFour(currentSubCategoryThreeId);
    _scrollToSelected();
  }

  Future<void> _fetchSubCategoryFour(int id) async {
    setState(() {
      isLoading = true;
    });
    try {
      final result = await ApiServices().getSubCategoryFour(id);
      
      final subCategories = result['sub_category_four'] ?? [];
      final products = result['products'] ?? [];
      
      if (subCategories.isEmpty && products.isEmpty) {
        // Auto-redirect to ProductScreen if there are no subcategories and no products
        Get.off(() => ProductScreen(
          categoryId: widget.categoryId,
          subcategoryId: widget.subCategoryId,
          subSubCategoryId: widget.subSubCategoryId,
          subCategoryThreeId: id,
          subcategoryName: currentSubCategoryThreeName,
        ));
        return;
      }

      setState(() {
        subCategoryFourList = subCategories;
        ...
        children: [
          ...
          Expanded(
            child: isLoading
                ...
                : ListView(
                    children: [
                      if (subCategoryFourList.isNotEmpty) ...[
                        Container(
                            ...
                            children: [
                              ...
                              SizedBox(
                                ...
                                child: ListView.builder(
                                  ...
                                  itemCount: subCategoryFourList.length,
                                  itemBuilder: (context, index) {
                                    final item = subCategoryFourList[index];
                                    ...
                                    return GestureDetector(
                                      onTap: () {
                                        Get.to(() => ProductScreen(
                                          ...
                                          subcategoryId: widget.subCategoryId,
                                          subSubCategoryId: widget.subSubCategoryId,
                                          subCategoryThreeId: currentSubCategoryThreeId,
                                          subCategoryFourId: item['id'],
                                          subcategoryName: item['name'] ?? item['subcategory'] ?? "Products",
                                          siblingCategories: subCategoryFourList,
                                        ));
                                      },
                                      child: Container(
                                        width: 125,
                                        ...
                                        child: ClipRRect(
                                          ...
                                          child: Stack(
                                            ...
                                            children: [
                                              Builder(
                                                builder: (context) {
                                                  String img = item['sub_cat_4_image'] ?? item['image'] ?? '';
                                                  String url = img.startsWith('http') ? img : "${imgPath}subcategoryfour/$img";
                                                  url = url.replaceAll(' ', '%20');
                                                  if (url == "${imgPath}subcategoryfour/" || url.isEmpty) {
                      ...
                      if (productList.isNotEmpty) ...[
                        ...
                        const SizedBox(height: 20),
                      ] else if (subCategoryFourList.isEmpty) ...[
                        const Padding(
